...
void main() {
  ...
  group('InvestmentNotifier - unarchiveInvestment', () {
    ...
    test('should move cash flows back when unarchiving', () async {
      ...
      // Verify cash flows moved to archived
      expect(fakeRepository.archivedCashFlows, hasLength(2));
      expect(fakeRepository.cashFlows, isEmpty);

      await notifier.unarchiveInvestment(created.id);

      // Verify cash flows moved back
      expect(fakeRepository.cashFlows, hasLength(2));
      expect(fakeRepository.archivedCashFlows, isEmpty);
    });
  });

  group('InvestmentNotifier - deleteArchivedInvestment', () {
    test('should delete an archived investment', () async {
      final notifier = container.read(investmentNotifierProvider.notifier);
      final created = await notifier.addInvestment(
        name: 'Test Investment',
        type: InvestmentType.p2pLending,
      ...
      await notifier.deleteArchivedInvestment(created.id);

      expect(fakeRepository.archivedInvestments, isEmpty);
    });

    test('should delete archived investment and its cash flows', () async {
      final notifier = container.read(investmentNotifierProvider.notifier);
      final created = await notifier.addInvestment(
        name: 'Test Investment',
        type: InvestmentType.p2pLending,
  ...
  group('InvestmentNotifier - archiveInvestment with cash flows', () {
    test('should move all cash flows to archived collection', () async {
      final notifier = container.read(investmentNotifierProvider.notifier);
      final created = await notifier.addInvestment(
        name: 'Test Investment',
        type: InvestmentType.p2pLending,
      ...
      expect(fakeRepository.cashFlows, hasLength(3));

      await notifier.archiveInvestment(created.id);

      // All cash flows should be moved
      expect(fakeRepository.cashFlows, isEmpty);
