...
void main() {
  ...
  group('SecurityService - PIN Management', () {
    ...
    test('setPin stores salted hashed PIN', () async {
      ...
      expect(storedValue, contains(':')); // Check for salt separator
      ...
      // Verify the hash is correct for the extracted salt
      final expectedHash = sha256.convert(utf8.encode(salt + pin)).toString();
      expect(hash, equals(expectedHash));
    });

    test('verifyPin returns true for correct PIN (salted hashed storage)', () async {
      await service.setPin('1234');
    ...
    test('verifyPin upgrades legacy plaintext PIN to salted hash', () async {
      ...
      // Check it is NOT plaintext anymore
      expect(storedValue, isNot(equals(plaintextPin)));
    });

    test('verifyPin upgrades legacy unsalted hash to salted hash', () async {
      // Setup: Manually store an unsalted hash (legacy state)
      const pin = '1234';
      ...
      final storedValue = fakeSecureStorage.storage['user_pin'];
      expect(storedValue, contains(':'));
      expect(storedValue, isNot(equals(unsaltedHash)));
    });
