...
## 2026-04-16 - Add localized tooltips to icon-only buttons
...
## 2024-03-24 - Interactive Card Accessibility and Nested Semantics
**Learning:** When adding Semantics to complex, grouped interactive widgets like cards, blindly wrapping the entire widget in a `GestureDetector` with `Semantics` can lead to nested interaction targets or failure to coalesce if an inner widget (like `GlassCard`) also attempts to handle `onTap` or `onLongPress`. If a cohesive container widget (like `GlassCard`) supports a `semanticLabel` natively, it correctly configures the single unified accessible focus point and enables `excludeSemantics` seamlessly.
**Action:** When working with visually grouped interactive text elements inside custom styled containers, utilize the container's built-in `semanticLabel` parameter to enforce cohesiveness, rather than wrapping it with disjointed generic `GestureDetector`s and hoping it merges automatically.
