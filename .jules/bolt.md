...
## 2026-05-10 - O(1) Median Calculation and Single-Pass Metrics
**Learning:** Computing the average, median, and counts of a dataset using chained functional operators like `.map().toList()`, `.reduce()`, `.sort()`, and `.where().length` is extremely inefficient. It results in multiple list allocations, redundant O(N) iterations, and unnecessary O(N log N) sorting.
**Action:** Calculate the total and matching counts in a single O(N) `for` loop. If the median is required, and a pre-sorted list of the data already exists in the same scope, compute the median in O(1) time by directly accessing the middle element(s) of the pre-sorted list.
