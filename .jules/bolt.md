...
## 2025-02-28 - Optimizing statistical metrics in Dart by replacing chained iterables with linear scans\n**Learning:** Chaining `.map().toList()`, `.reduce()`, `.sort()`, and `.where().length` causes excessive list allocations and O(N log N) overhead, especially in core analytical services where datasets can grow large.\n**Action:** When computing sums, averages, medians, and counts simultaneously, compute the totals and conditional counts within a single O(N) `for` loop. Additionally, if the data is already sorted by a relevant metric, use it to calculate the median in O(1) time without allocating and sorting a redundant array.
