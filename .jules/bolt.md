...
## 2026-05-08 - Pre-grouping Map Iterations
**Learning:** When generating time-series data from collections in Dart, nested loop iterations (e.g., calling `.where().toList()` on an entire dataset for every single day in a period) creates an O(N*M) bottleneck.
**Action:** Always pre-calculate groups using a dictionary comprehension or a single-pass loop to populate a `Map` (e.g., `Map<String, List<Entity>>` using a formatted date key), reducing the nested inner loop to an O(1) map lookup, changing overall complexity to O(N+M).
