...
## 2026-05-07 - Replace O(N*M) daily cache flow lookups with O(1) Map
**Learning:** Generating multiple sequential  filters inside a `while` loop to extract daily cash flows iterates through the entire dataset repeatedly, causing severe O(days * N) time complexity.
**Action:** Iterate through the dataset exactly once before the `while` loop to group items by a string key (like a date string `'${date.year}-${date.month}-${date.day}'`) into a `Map<String, List<Type>>`. Then inside the loop, retrieve elements using O(1) Map lookups, bringing complexity down to O(days + N).
## 2026-05-07 - Replace O(N*M) daily cache flow lookups with O(1) Map
**Learning:** Generating multiple sequential where().toList() filters inside a while loop to extract daily cash flows iterates through the entire dataset repeatedly, causing severe O(days * N) time complexity.
**Action:** Iterate through the dataset exactly once before the while loop to group items by a string key (like a date string YYYY-MM-DD) into a Map. Then inside the loop, retrieve elements using O(1) Map lookups, bringing complexity down to O(days + N).
