...
## 2026-07-03 - Avoid O(N log N) sorting and string parsing in comparator
**Learning:** In `SmartInsightsService._generateDecliningAlerts`, the code sorted the entire `insights` list by parsing string percentages within the `.sort()` comparator, just to extract the top 3 items using `.take(3).toList()`. This is an O(N log N) bottleneck, exacerbated by heavy string manipulations and double parsing during each comparison step.
**Action:** Replace the post-processing `.sort()` and `.take(3)` with an O(N) bounded list insertion during the iteration. By maintaining a maximum list size of 3 and inserting items in sorted order as they are found, we eliminate the O(N log N) sorting overhead entirely.
