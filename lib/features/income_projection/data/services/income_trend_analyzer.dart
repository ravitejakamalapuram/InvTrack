...
class IncomeTrendAnalyzer {
  ...
  List<MonthlyIncomeData> _calculateMonthlyData(
    ...
    for (final cf in cashFlows) {
      ...
      final monthsDiff =
          (now.year - cf.date.year) * 12 + (now.month - cf.date.month);
  ...
  double _calculateQoQGrowth(List<MonthlyIncomeData> monthlyData) {
    ...
    final currentQ = monthlyData
        .skip(monthlyData.length - 3)
  ...
  List<PlatformReliability> _calculatePlatformReliability(
    ...
    for (final expected in expectedCashFlows) {
      ...
      final investment =
          investmentMap[expected.investmentId] ?? investments.first;
    ...
    }).toList()..sort((a, b) => b.onTimeRate.compareTo(a.onTimeRate));
  }
  ...
  List<IncomeSource> _calculateIncomeSources(
    ...
    final totalIncome = investmentIncomeMap.values.fold<double>(
      0.0,
      (a, b) => a + b,
    );
    ...
    return investmentIncomeMap.entries.map((e) {
      final investment = investmentMap[e.key] ?? investments.first;
    ...
    }).toList()..sort((a, b) => b.totalIncome.compareTo(a.totalIncome));
  }
  ...
  double _calculateHHI(List<IncomeSource> sources) {
    ...
    final hhi = sources.fold<double>(0.0, (sum, source) {
  ...
  List<String> _generateInsights({
    ...
    if (momGrowth > 10) {
      insights.add(
        'Strong growth! Your income grew ${momGrowth.toStringAsFixed(1)}% last month.',
      );
    } else if (momGrowth > 0) {
      insights.add(
        'Positive growth: ${momGrowth.toStringAsFixed(1)}% increase last month.',
      );
    } else if (momGrowth < -10) {
      insights.add(
        'Income declined ${momGrowth.abs().toStringAsFixed(1)}% last month. Review your investments.',
      );
    }
    ...
    if (diversificationScore > 0.50) {
      ...
      if (topSource != null) {
        insights.add(
          '${topSource.percentage.toStringAsFixed(0)}% of income from ${topSource.investmentName}. Consider diversifying.',
        );
      }
    } else if (diversificationScore < 0.15) {
      insights.add(
        'Excellent diversification! Your income is well-balanced across investments.',
      );
    }

    // Platform reliability insights
    if (platformReliability.isNotEmpty) {
      final unreliable = platformReliability
          .where((p) => p.onTimeRate < 0.80)
          .toList();
      if (unreliable.isNotEmpty) {
        final platform = unreliable.first;
        insights.add(
          '${platform.platform} is frequently late (${(platform.onTimeRate * 100).toStringAsFixed(0)}% on-time rate).',
        );
      }

      final reliable = platformReliability
          .where((p) => p.onTimeRate >= 0.95)
          .toList();
      if (reliable.isNotEmpty) {
        insights.add(
          '${reliable.length} platforms with excellent reliability (95%+ on-time).',
        );
      }
    }
