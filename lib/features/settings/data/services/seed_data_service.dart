...
class SeedDataService {
  ...
  Future<({int investments, int cashFlows})> seedDemoData() async {
    ...
    final p2p = addInvestment(
      ...
      notes:
          'Diversified P2P portfolio with 50+ borrowers. Exited with 15% XIRR.',
    );
    addCashFlow(
      p2p.id,
      now.subtract(const Duration(days: 540)),
      CashFlowType.invest,
      50000,
    );
    addCashFlow(
    ...
    final realEstate = addInvestment(
      ...
      notes:
          '2BHK in Whitefield. Tenant: TechCorp Ltd. Lease expires Dec 2025.',
    );
    addCashFlow(
      realEstate.id,
    ...
    final gold = addInvestment(
      ...
      maturityDate: now.add(
        const Duration(days: 2555),
      ), // ~7 years remaining (8 year SGB)
    ...
    final angel = addInvestment(
      ...
      notes:
          'AI/ML startup in healthcare. 5% equity stake. Next funding round expected Q2 2025.',
    );
    addCashFlow(
      angel.id,
    ...
    addCashFlow(
      bonds.id,
      now.subtract(const Duration(days: 620)),
      CashFlowType.income,
      10500,
    );
    addCashFlow(
      bonds.id,
      now.subtract(const Duration(days: 440)),
      CashFlowType.income,
      10500,
    );
    addCashFlow(
    ...
    final mf = addInvestment(
      ...
      incomeFrequency: IncomeFrequency.monthly,
      notes:
          'UTI Nifty 50 Index Fund. SIP of ₹10,000/month. Long-term wealth building.',
    );
    for (int i = 12; i >= 1; i--) {
      addCashFlow(
        mf.id,
