...
class XirrSolver {
  ...
  static double _f(double x, List<double> yearsFromStart, List<double> amounts) {
    ...
    for (int i = 0; i < amounts.length; i++) {
      ...
      // Optimization: Skip expensive exp calculation for initial cash flow (power=0)
      if (power == 0) {
        sum += amounts[i];
        continue;
      }
  ...
  static (double, double) _calculateFandDf(
    ...
    for (int i = 0; i < amounts.length; i++) {
      ...
      // Optimization: Skip expensive exp calculation for initial cash flow (p=0)
      // exp(0) = 1, and df term becomes 0 because p=0
      if (p == 0) {
        fSum += amounts[i];
        continue;
      }
      ...
      fSum += termF;
      // Optimization: Moved invBase multiplication outside loop (applied at return)
      // This saves N multiplications per iteration
      dfSum += termF * (-p);
    }
