...
* **ref.read in build methods**: Usage of `ref.read` directly inside `build()` methods in widgets such as `lib/features/settings/presentation/screens/data_management_screen.dart` violates Riverpod best practices (Rule 14.1) and causes stale state issues.
...
1. Top 10 highest-value fixes.
   ...
   8. Split `lib/core/notifications/notification_service.dart` to reduce its footprint.
   9. Verify all `Semantics` wrappers on interactive custom elements explicitly define `onTap`.
   10. Remove redundant `await` operations on synchronous data within loops.
