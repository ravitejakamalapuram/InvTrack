...
## Executive Summary
* **Overall Repo Health Score**: 65/100 (Technical debt in critical presentation layer files undermines baseline resilience and performance constraints).
...
* **CSV Parsing Logic**: Bulk import features (`lib/features/bulk_import/data/services/simple_csv_parser.dart`) contain manual iteration and error extraction that could be centralized in a shared domain utility.
...
* **Rate Limiting & Security State**: When credentials (like PINs) are removed, associated security states such as failed attempt counters and lockout timestamps must be explicitly cleared to prevent rate-limiting bypasses (e.g. in `lib/core/services/security_service.dart`).
...
* **Rule 19.5 (Offline Behavior)**: Missing `.timeout(Duration(seconds: 5))` on several Firestore writes in `lib/features/investment/data/repositories/firestore_investment_repository.dart`.
...
* Apply `.timeout(Duration(seconds: 5))` to all missing Firestore write operations across repository classes.
* Refactor unoptimized list iterations (replace chained `.where().toList()` and `.sort()` max/min scans with single-pass `for` loops).
* Consolidate `OverviewEmptyState`, `GoalsEmptyState`, and `InvestmentEmptyState` into a unified `AppEmptyState` widget.

### Medium Effort (2-4 weeks)
* Split `lib/features/investment/presentation/screens/add_investment_screen.dart` and `lib/features/investment/presentation/widgets/add_document_sheet.dart` into smaller, independent sub-form widgets to reduce file sizes.
* Centralize application exception handling to exclusively use `ErrorHandler.handle()` instead of local try-catch mapping.
* Establish a standard `AppFormField` abstraction and migrate all hardcoded text field configurations.

### Long-term Architecture (1-2 months)
* Break down `lib/core/analytics/analytics_service.dart` and `lib/core/notifications/notification_service.dart` into specialized domain providers focused on distinct entities.
* Implement strict linting rules or CI checks to fail builds on files exceeding 500 lines.
* Migrate all data transformation and formatting logic strictly into domain layer providers to purify the presentation layer.
...
1. Top 10 highest-value fixes.
   ...
   - Apply `.timeout(Duration(seconds: 5))` to all missing Firestore write operations in `lib/features/investment/data/repositories/firestore_investment_repository.dart`.
   ...
   - Centralize exception mapping using the core `ErrorHandler` in `lib/core/error/error_handler.dart`.
