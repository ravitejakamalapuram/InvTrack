...
* **Highest ROI Improvements**: Refactoring `lib/features/investment/presentation/screens/add_investment_screen.dart` and `lib/core/analytics/analytics_service.dart` into smaller components. Ensuring all `batch.set` and `.add` Firestore operations use `.timeout()`. Consolidating duplicated empty state widgets.
...
## Testing Gaps
* **Oversized Component Tests**: Massive files (`lib/core/notifications/notification_service.dart`) result in brittle or incomplete tests.
...
* **Rule 19.5 (Offline Behavior)**: Missing `.timeout(Duration(seconds: 5))` on Firestore writes (`lib/features/goals/data/repositories/firestore_goal_repository.dart`, `lib/features/investment/data/repositories/firestore_investment_repository.dart`).
...
3. Consolidate `lib/features/overview/presentation/widgets/overview_empty_state.dart`, `lib/features/goals/presentation/widgets/goals_empty_state.dart`, and `lib/features/investment/presentation/widgets/investment_list_states.dart` into `AppEmptyState`.

### Medium Effort (2-4 weeks)
1. Split `lib/features/investment/presentation/screens/add_investment_screen.dart` and `lib/features/investment/presentation/widgets/add_document_sheet.dart` into smaller, independent sub-form widgets.
...
1. Break down `lib/core/analytics/analytics_service.dart` and `lib/core/notifications/notification_service.dart` into domain-specific providers.
...
1. Top 10 highest-value fixes
  1. Apply `.timeout(Duration(seconds: 5))` to all missing Firestore write operations (e.g. `lib/features/investment/data/repositories/firestore_investment_repository.dart`).
