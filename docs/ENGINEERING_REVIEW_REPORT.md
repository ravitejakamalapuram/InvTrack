...
## Executive Summary
* **Overall Repo Health Score**: 6.5/10. The application exhibits a solid feature-first architecture and relies on modern Flutter/Dart practices (e.g., Riverpod, Freezed), but struggles with significant code duplication, oversized components, and missing offline resiliency.
...
* **Missing Exception Handling Mapping**: Core services handle exceptions natively instead of consistently routing them through the centralized `ErrorHandler.handle()` method in `lib/core/error/error_handler.dart`.
...
* **CSV Parsing Logic**: Bulk import features contain manual iteration and error extraction in `lib/features/bulk_import/data/services/simple_csv_parser.dart` that could be centralized in a shared domain utility.
* **Form Field Configurations**: UI screens re-declare consistent form styles, spacings, and validation for text fields and dropdowns instead of utilizing shared abstractions (e.g., in `lib/features/investment/presentation/screens/add_investment_screen.dart`).

## Reusability Opportunities
* **`AppEmptyState` Component**: Create a single parameterized abstraction for all empty states (accepting an icon, title, description, and primary action) in `lib/core/widgets/`.
* **`AppFormField` / `ValidatedTextField`**: A reusable builder that standardizes form inputs to ensure uniform styling and validation across all screens in `lib/core/widgets/`.
...
* **Rule 19.5 (Offline Behavior)**: Missing `.timeout(Duration(seconds: 5))` on several Firestore writes in repositories like `lib/features/investment/data/repositories/firestore_investment_repository.dart`.
...
1. Apply `.timeout(Duration(seconds: 5))` to all missing Firestore write operations across repository classes like `lib/features/investment/data/repositories/firestore_investment_repository.dart`.
...
2. Apply `.timeout(Duration(seconds: 5))` to all missing Firestore write operations in `lib/features/investment/data/repositories/firestore_investment_repository.dart`.
...
5. Consolidate empty state UI implementations (`GoalsEmptyState`, `OverviewEmptyState`, `InvestmentEmptyState`) into a single `AppEmptyState` widget.
6. Replace full-list sorting for extremum findings with O(N) linear scans.
7. Centralize exception mapping using the core `ErrorHandler.handle()` in `lib/core/error/error_handler.dart`.
...
1. Empty state UI widgets (`lib/features/goals/presentation/widgets/goals_empty_state.dart`, `lib/features/overview/presentation/widgets/overview_empty_state.dart`, `lib/features/investment/presentation/widgets/investment_list_states.dart`).
2. Form field layout and validation logic across screens (`lib/features/investment/presentation/screens/add_investment_screen.dart`).
3. Exception-catching and mapping blocks inside individual repositories.
4. Loading skeletons and shimmering effect containers in `lib/core/widgets/loading_skeletons.dart`.
5. Date and number formatting patterns duplicated across presentation widgets.
6. Bottom sheet presentation configurations in `lib/features/investment/presentation/widgets/add_document_sheet.dart`.
7. Reused prompt/dialog structures for confirmations.
8. Theme color extraction patterns across the app.
9. Toast/Snackbar notification displays in `lib/core/notifications/notification_service.dart`.
